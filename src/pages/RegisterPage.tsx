...
export default function RegisterPage() {
  ...
  const go = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name || !email || !pw) { setErr('Fylltu út alla reiti'); return; }
    if (pw.length < 6) { setErr('Lykilorð þarf að vera a.m.k. 6 stafir'); return; }
    ...
    try {
      ...
      if (err?.code === 'auth/email-already-in-use') {
        ...
        setErr('Lykilorð er of veikt (a.m.k. 6 stafir)');
      } else if (err?.code === 'auth/invalid-email') {
        setErr('Ógilt netfang');
      } else {
        setErr('Villa við nýskráningu');
      }
