...
export default function LoginPage() {
  ...
  const go = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email) { setErr('Sláðu inn netfang'); return; }
    if (!pw) { setErr('Sláðu inn lykilorð'); return; }
    ...
    try {
      ...
      if (err?.code === 'auth/invalid-credential' || err?.code === 'auth/user-not-found') {
        setErr('Rangt netfang eða lykilorð');
      } else if (err?.code === 'auth/too-many-requests') {
        setErr('Of margar tilraunir. Reyndu aftur síðar.');
      } else {
        setErr('Villa við innskráningu');
      }
  ...
  const handleApple = async () => {
    // TODO: Wire up Apple sign-in when configured in Firebase
    setErr(''); setLoading(true);
    try {
      await login('apple-user@icloud.com', 'apple');
      nav('/');
    } catch {
      setErr('Apple innskráning er ekki enn virk');
    } finally { setLoading(false); }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-brand-600 to-brand-800 flex items-center justify-center px-4">
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6 space-y-4">
          ...
          {/* Social logins */}
          <button onClick={handleGoogle} type="button" disabled={loading} className="w-full py-3 border border-gray-200 rounded-xl text-sm font-medium hover:bg-gray-50 flex items-center justify-center gap-3 disabled:opacity-50">
          ...
          <button onClick={handleApple} type="button" disabled={loading} className="w-full py-3 bg-black text-white rounded-xl text-sm font-medium hover:bg-gray-900 flex items-center justify-center gap-3 disabled:opacity-50">
