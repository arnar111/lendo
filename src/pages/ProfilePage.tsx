    ...
    <div className="min-h-screen bg-gray-50 pb-20">
      <header className="bg-white border-b sticky top-0 z-40 px-4 py-3">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          ...
          {isOwn && <div className="flex gap-2">
            <button onClick={() => nav('/tekjur')}><TrendingUp size={20} className="text-teal-600" /></button>
      ...
      <main className="max-w-lg mx-auto px-4 py-4">
        <div className="flex items-center gap-4 mb-6">
          <div className="w-20 h-20 rounded-full bg-teal-100 flex items-center justify-center text-teal-700 font-bold text-2xl overflow-hidden shrink-0">{pu.photoURL ? <img src={pu.photoURL} className="w-full h-full object-cover" /> : pu.displayName.charAt(0)}</div>
          <div>
        ...
        {isOwn && (
          <button onClick={() => nav('/tekjur')} className="w-full bg-gradient-to-r from-teal-600 to-emerald-600 text-white rounded-2xl p-4 mb-6 flex items-center gap-3 hover:from-teal-700 hover:to-emerald-700 transition-all shadow-sm">
            <div className="w-12 h-12 bg-white/20 backdrop-blur rounded-xl flex items-center justify-center"><TrendingUp size={24} /></div>
            <div className="text-left flex-1"><p className="font-bold">Tekjuyfirlit</p><p className="text-sm text-teal-100">Sjáðu tekjur þínar og tölfræði</p></div>
        ...
        {editing && <div className="bg-white rounded-xl border p-4 mb-6 space-y-3">
          ...
          <button onClick={() => { updateProfile({ displayName: eName || pu.displayName, bio: eBio }); setEditing(false); }} className="px-4 py-2 bg-teal-600 text-white rounded-lg text-sm font-medium">Vista</button>
        </div>}

        <div className="mb-6">
          <h3 className="font-semibold mb-3">Auglýsingar ({uItems.length})</h3>
          {uItems.length === 0 ? <div className="text-center py-8 bg-white rounded-xl border"><p className="text-gray-500 text-sm">Engar auglýsingar</p>{isOwn && <Link to="/skra-hlut" className="text-teal-600 text-sm font-medium mt-2 inline-block">+ Skrá hlut</Link>}</div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3"><h3 className="font-semibold">Umsagnir sem eigandi</h3>
            {me && !isOwn && <button onClick={() => setShowRF(!showRF)} className="text-sm text-teal-600 font-medium">+ Skrifa</button>}</div>
          {showRF && <div className="bg-white rounded-xl border p-4 mb-4"><StarRating rating={rr} onChange={setRR} /><textarea value={rt} onChange={e => setRT(e.target.value)} className="w-full mt-2 p-2 border rounded-lg text-sm resize-none h-20" /><button onClick={() => { if (!me) return; addReview({ authorId: me.uid, authorDisplayName: me.displayName, targetType: 'user_owner', targetId: pu.uid, rating: rr as 1|2|3|4|5, text: rt || undefined }); setShowRF(false); setRT(''); }} className="mt-2 px-4 py-2 bg-teal-600 text-white rounded-lg text-sm font-medium">Senda</button></div>}
