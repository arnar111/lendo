      ...
      <div className="w-full max-w-sm">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-6">
          ...
          {step === 0 && (
            <div className="space-y-4 animate-fade-in-up">
              <div className="text-center mb-2">
                ...
                <h2 className="text-xl font-bold">Velkomin á Lendó!</h2>
              ...
              <input
                ...
                autoFocus
                className="w-full px-4 py-3 rounded-xl bg-gray-100 text-sm border-0 focus:ring-2 focus:ring-brand-500 text-center text-lg font-medium"
              />
              <button
                onClick={() => { if (name.trim()) setStep(1); }}
                disabled={!name.trim()}
                className="w-full py-3 bg-brand-600 text-white rounded-xl font-semibold text-sm disabled:opacity-40"
              >
                Áfram
              </button>
            </div>
            ...
            <div className="space-y-4 animate-fade-in-up">
              ...
              <div className="flex justify-center">
                <div className="relative">
                  <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-brand-100 shadow-lg">
                    <img src={currentPhoto} alt="" className="w-full h-full object-cover" />
                  </div>
                  <button
                    onClick={() => fileRef.current?.click()}
                    className="absolute -bottom-1 -right-1 w-8 h-8 bg-brand-600 text-white rounded-full flex items-center justify-center shadow-md"
                  >
                    <Camera size={14} />
                  </button>
                </div>
                ...
                <div className="grid grid-cols-6 gap-2">
                  {AVATAR_SEEDS.map(seed => (
                    <button
                      ...
                      className={`w-12 h-12 rounded-full overflow-hidden border-2 transition-all ${selectedAvatar === seed && !useCustomPhoto ? 'border-brand-500 ring-2 ring-brand-200 scale-110' : 'border-gray-200 hover:border-brand-300'}`}
                    >
              ...
              <div className="flex gap-2">
                ...
                <button onClick={() => setStep(2)} className="flex-1 py-3 bg-brand-600 text-white rounded-xl font-semibold text-sm">Áfram</button>
              </div>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-4 animate-fade-in-up">
              <div className="text-center mb-2">
                <div className="w-16 h-16 rounded-full overflow-hidden mx-auto mb-3 border-2 border-brand-100">
                  <img src={currentPhoto} alt="" className="w-full h-full object-cover" />
                </div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 rounded-xl bg-gray-100 text-sm border-0 focus:ring-2 focus:ring-brand-500 resize-none h-20"
              />
              ...
              <div className="flex gap-2">
                ...
                <button onClick={finish} className="flex-1 py-3 bg-brand-600 text-white rounded-xl font-semibold text-sm flex items-center justify-center gap-2">
                  <Check size={18} /> Klára
                </button>
              </div>
            </div>
